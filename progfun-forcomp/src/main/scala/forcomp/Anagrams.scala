...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toList.map(_.toLower).groupBy((element: Char) => element).toList.map( e => (e._1, e._2.length)).sortWith(_._1 < _._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (c, n) :: rest =>
      for {
    	  comb <- (0 to n).map(x => (c, x)).toList
    	  combRest <- combinations(rest)
      } yield (comb :: combRest).filter(x => x._2 != 0)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = 
    x.map( a => y.find(b => a._1 == b._1) match {
    ...
    def helper(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case _ =>
        for {
        	...
        	rest <- helper(subtract(occurrences, comb))
        } yield words::rest
    }
